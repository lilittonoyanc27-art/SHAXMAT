...
export default function ChessGrammarGame() {
  ...
  const makeComputerMove = (currentGame: Chess) => {
    ...
    if (moves.length > 0) {
      ...
      setGame(gameCopy);
      setMoveStatus('Մրցակիցը կատարեց իր քայլը: Սեղմիր «Հաջորդ հարցը» կոճակը:');
    } else {
      setGame(gameCopy);
    }
    setIsMoving(false);
  ...
  const handleOptionClick = (option: string) => {
    ...
    if (game.turn() === 'w') {
      setMoveCredit(true);
      setMoveStatus('Պատասխանը տրված է: Այժմ կատարիր քո քայլը (անկախ պատասխանից):');
    } else {
      setMoveCredit(false);
      setMoveStatus('Պատասխանը տրված է: Մրցակիցը կատարում է իր քայլը...');
      setIsMoving(true);
        ...
        <div className="lg:col-span-12 flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-[2.5rem] shadow-xl border-2 border-orange-100 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 bg-orange-50 px-3 py-1 rounded-full w-fit mt-1 border border-orange-100">
                <Bird className="w-5 h-5 text-orange-500" />
                <span className="text-xs font-black text-orange-700 uppercase tracking-wider">Jose</span>
              </div>
              <p className="text-orange-600 font-bold text-sm flex items-center gap-2 mt-2">
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border-2 border-orange-100 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-10">
                <Bird className="w-24 h-24 text-orange-500" />
              </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        {(feedback && !moveCredit && !isMoving && !isGameOver) && (
                          <button
                            onClick={() => {
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white p-4 rounded-3xl border-2 border-orange-100 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-bold text-slate-900">Хосе</p>
                </div>
              </div>
            </div>
          </div>
